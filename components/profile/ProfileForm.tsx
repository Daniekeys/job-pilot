...
export function ProfileForm({ profile, onChange: setProfile }: Props) {
  ...
  return (
    <div className="rounded-2xl border border-border bg-surface p-6 shadow-[0px_1px_3px_rgba(0,0,0,0.1),0px_1px_2px_-1px_rgba(0,0,0,0.1)]">
      ...
      <fieldset
        disabled={isLocked}
        className="mt-8 flex flex-col gap-8 border-0 p-0"
      >
        {/* Personal Info */}
        <div>
        ...
        <div>
          ...
          {profile.workExperience.map((role, index) => (
            <div
              key={index}
              className={
                index > 0 ? "mt-6 border-t border-border pt-6" : "mt-4"
              }
            >
          ...
          <div className={`mt-4 ${fieldGridClass}`}>
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  updateEducation("highestDegree", e.target.value)
                }
              >
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  updateEducation("institutionName", e.target.value)
                }
              />
            </div>
            <div>
        ...
        <div>
          ...
          <div className="mt-4">
            ...
            <input
              className={inputClass}
              value={profile.jobTitlesSeeking.join(", ")}
              onChange={(e) =>
                updateField(
                  "jobTitlesSeeking",
                  e.target.value
      ...
      {status && (
        <p
          className={`mt-4 text-sm ${status.type === "success" ? "text-success" : "text-error"}`}
        >
          {status.message}
        </p>
      )}
