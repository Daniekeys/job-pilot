...
Extract from Resume button — gpt-5.4-mini reads uploaded PDF and auto-fills profile form fields.
...
- gpt-5.4-mini reads extracted text and returns structured JSON matching all profile field names
...
Generate a clean professional PDF resume from current profile data using gpt-5.4-mini.

**Logic:**

- POST /api/resume/generate
- Reads current profile data from profiles table
- gpt-5.4-mini generates professional resume content:
  - Professional summary paragraph
  - Polished work experience bullet points
  - Clean professional language throughout
- @react-pdf/renderer renders gpt-5.4-mini output into clean single-page PDF using renderToBuffer()
...
- For each job returned:
  ...
  - gpt-5.4-mini scores job against user profile:
...
- AI Match Reasoning section — match reason paragraph from gpt-5.4-mini
...
- Derive company homepage URL by following the Adzuna redirect with server-side fetch() — no browser needed for this step:
  ...
  - If Stagehand gets no meaningful content (oneLiner and productSummary empty) — skip browser research entirely, proceed to gpt-5.4-mini synthesis with job description and profile only
...
- Close Browserbase session after homepage + max 3 sub-pages
  **gpt-5.4-mini synthesis (runs after browser closes):**
...
- Always return a dossier — never fail silently. If browser research failed, gpt-5.4-mini synthesizes from job description and profile alone.
