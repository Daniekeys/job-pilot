...
Plain HTML `<table>`, not a shadcn data-table — matches ui-rules.md's Table spec directly (`border-b border-border` between header and rows, no header `border-b` on the last row, `hover:bg-surface-secondary` per row, `cursor-pointer` to signal future clickability even though navigation to `/find-jobs/[id]` isn't wired yet — that route doesn't exist until Phase 4). Column headers: `px-4 py-3 text-left text-xs font-medium uppercase tracking-wide text-text-secondary`. Row cells: `px-4 py-3`, primary text `text-sm font-medium text-text-primary` (company, match %), secondary `text-sm text-text-primary` (role) or `text-sm text-text-secondary` (salary), muted `text-sm text-text-muted` (date found). Company icon is a generic `Building2` (`lucide-react`) inside `flex size-8 items-center justify-center rounded-md bg-surface-tertiary` — there are no real company logos yet (Adzuna doesn't provide them), this placeholder pattern should be reused anywhere else a company icon is needed until real logos exist.
...
**Feature 12 addition:** `JobsTable` is now `"use client"` — each row has `onClick={() => router.push(\`/find-jobs/${job.id}\`)}`, closing the navigation gap this file's own note above had deferred ("isn't wired yet — that route doesn't exist until Phase 4"). No new keyboard/`role` semantics added beyond the existing `cursor-pointer` styling.
...
Card with a header row (`border-b border-border`) containing the heading (same larger style as `JobDescription`, `size-9 rounded-lg bg-accent-muted` `Building2` icon chip — pixel-matched) and a "Research Company" button (`bg-accent text-accent-foreground rounded-md` — the project's standard primary-button classes, plus a leading `Search` icon). **Button has no `onClick`** — empty-state-only per build-plan.md's explicit scope line for feature 12 ("Company Research section shows empty state only"), same "ship the UI inert, wire it next feature" precedent as `ResumeUpload`'s buttons before 06/07/08. Empty state below: centered `size-12 rounded-xl bg-surface-tertiary` `Building2` icon, "No research yet", and copy that references the real `job.company` name. Feature 13 adds the `onClick` and the post-research dossier rendering — nothing in this component's structure needs to change for that, since it already takes the real `job` prop.
