...
Logged-in app navbar variant used on `/profile`, `/dashboard`, `/find-jobs`, `/find-jobs/[id]` — distinct from the marketing `Navbar` (no CTA button, three nav links with active-state highlighting). `"use client"` (needs `usePathname`, plus the user-menu state added below). Same `h-16 w-full border-b border-border bg-surface` shell and `max-w-[1440px] mx-auto px-6` inner container as `Navbar`, but right side is the three nav links (`Dashboard`, `Find Jobs`, `Profile`) plus the user menu. Active link (`pathname.startsWith(href)`): `text-sm font-medium text-accent`. Inactive: `text-sm font-medium text-text-dark hover:text-accent` — same inactive color as marketing `Navbar` (`text-text-dark`, not the `#4A5565` literal in ui-rules.md, since that hex has no matching token and the codebase already established this color for inactive nav links).

**User menu addition (no design mockup exists for this — built from ui-rules.md conventions, developer-requested):** takes a required `userEmail: string` prop — every caller already has `user.email` from its own auth check, so no extra DB query was needed anywhere. Trigger is a `size-8 rounded-full bg-accent-light text-accent` circle showing the email's first letter (uppercased) + a `ChevronDown` (lucide). Clicking opens a dropdown (`absolute right-0 mt-2 w-56 rounded-md border border-border bg-surface shadow-[...] py-1`, same shadow/border/radius as `SearchControls`' suggestion dropdown) showing the full email (`truncate border-b border-border px-3 py-2 text-sm font-medium`) and a "Sign Out" row. **Reuses the existing `signOutAction` from `actions/auth.ts`** (built in feature 02, never had a UI control calling it until now) via the same `<form action={signOutAction}>` + submit-`<button>` pattern as `OAuthButton`. **Close-on-blur uses the exact same `BLUR_CLOSE_DELAY_MS` (150ms) pattern as `SearchControls`' suggestion dropdown** — blur on the trigger button closes the menu after a short delay, and the "Sign Out" button has `onMouseDown={(e) => e.preventDefault()}` so clicking it never blurs the trigger in the first place. Every page rendering `AppNavbar` now passes `userEmail={user.email ?? ""}`.
...
**Feature 15 (Real Data):** purely presentational now — takes a `stats: DashboardStat[]` prop, no hardcoded array left inside the component. `app/dashboard/page.tsx` fetches `match_score, found_at, company_research` for all of the current user's jobs and passes the result through `computeDashboardStats()` (`lib/dashboard-stats.ts`), which derives total count, average match score, company-research count, and last-7-days count, plus the two trend pills as real week-over-week comparisons (computed from the same fetched rows — no second query). Trend renders as `undefined` (no pill) whenever there's no 8–14-day-old comparison window yet, rather than showing a fabricated number — see `progress-tracker.md`'s 15 entry for the live-data verification.
...
**Feature 16 (Real Data):** resolved as predicted — the third (`accent`) dot color is dropped entirely, since only two real activity types exist. `RecentActivity` is now presentational, taking `activities: ActivityItem[]` (`lib/recent-activity.ts`'s exported type) instead of an internal mock array; `dot` is now `"info" | "success"` only. New empty state (`py-6 text-center text-sm text-text-muted`, "No activity yet — search for jobs to get started.") for brand-new users with zero `agent_runs`/researched jobs, same minimal pattern as `JobsTable`'s feature-10 empty state. Mapping: `agent_run` "Found X jobs" entries → `success` (matches ui-tokens.md's existing "Job found" Activity Dot row exactly); `company_research` "Researched {company}" entries → `info` (the only color build-plan.md's "info blue, success green" wording leaves for the second type).
...
**Feature 17:** takes a `data: AnalyticsData` prop (`lib/dashboard-analytics.ts`) instead of owning the three mock arrays. `ChartCard` gained `isEmpty`/`emptyMessage` props — per build-plan.md's explicit "empty state shown for each chart when no data exists yet" requirement, each card renders a centered `h-64 text-sm text-text-muted` message instead of an all-zero chart when every bucket's count is 0 (`YAxis` also gained `allowDecimals={false}` across all three charts, since job/research counts are always whole numbers). Jobs Found Over Time and Company Research Activity now use real calendar-date `day` labels ("Jun 21") instead of feature 14's mock weekday labels, since their real windows (30 days / 7 days) don't map cleanly onto 7 weekday names. **Data source pivoted from PostHog to the `jobs` table directly** (developer-directed mid-session — see `progress-tracker.md`'s 17 entry) — `AnalyticsData`'s shape didn't change, so this component itself required no edits, only `lib/dashboard-analytics.ts` changed underneath it.
