...
// Batches are capped at 10 jobs so the prompt stays within provider limits while
// preserving the original order.
const SCORING_MAX_TOKENS = 4000;
const MAX_JOBS_PER_SCORE_REQUEST = 10;
...
function formatSalary(job: AdzunaJob, country: string): string {
  ...
  const symbol = currencySymbolForCountry(country);
...
// Scores every job in batches to stay within provider limits without losing order.
async function scoreJobs(
  jobs: AdzunaJob[],
  profile: Profile,
): Promise<
  | { success: true; scores: z.infer<typeof scoreSchema>["scores"] }
  | { success: false; error: string }
> {
  const scores: z.infer<typeof scoreSchema>["scores"] = [];

  for (
    let start = 0;
    start < jobs.length;
    start += MAX_JOBS_PER_SCORE_REQUEST
  ) {
    const batch = jobs.slice(start, start + MAX_JOBS_PER_SCORE_REQUEST);
...
${batch.map((job, i) => `${i + 1}. ${job.title} at ${job.company.display_name}\n${job.description}`).join("\n\n")}`;
    ...
    if (result.data.scores.length !== batch.length) {
      return {
        success: false,
        error: "Scoring response length did not match job count",
      };
    }

    scores.push(...result.data.scores);
  }
  ...
  try {
    ...
    if (!scoring.success) {
      await logAgentError(
        runId,
        userId,
        `Job scoring failed: ${scoring.error}`,
      );
    }
    ...
    await logAgentError(
      runId,
      userId,
      `Adzuna discovery failed: ${String(error)}`,
    );
    return {
      success: false,
