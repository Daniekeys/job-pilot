...
const ADZUNA_SUPPORTED_REGIONS =
  "UK, US, Germany, France, Australia, New Zealand, Canada, India, Poland, Brazil, Austria, South Africa";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const jobTitle =
      inputJobTitle !== ""
        ? inputJobTitle
        : profile.jobTitlesSeeking[0] || profile.currentTitle.trim();
    ...
    if (inputCountry !== "" && isSupportedCountryCode(inputCountry)) {
      ...
      profileLocationText = profile.preferredLocations[0] ?? "";
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            "Enter a job title, or add a target title to your profile to search automatically.",
        },
        { status: 422 },
      );
    }
    ...
    if (result.jobs.length > 0) {
      ...
      if (insertError) {
        ...
        await insforge.database
          ...
          .eq("id", runId);
        return NextResponse.json(
          { success: false, error: "Failed to save discovered jobs." },
          { status: 500 },
        );
      }
    ...
    const strongMatches = result.jobs.filter(
      (job) => job.matchScore >= MATCH_THRESHOLD,
    ).length;
    return NextResponse.json({
      success: true,
      data: {
    ...
    // Best effort: mark the run as failed even if something throws after the insert.
    // This keeps the run from getting stuck in `running`.
    if (runId) {
      const insforge = await createInsforgeServer();
      await insforge.database
        .from("agent_runs")
