  ...
  try {
    ...
    if (!user) {
      return NextResponse.json(
        {
          success: false,
          error: "You must be signed in to generate a resume.",
        },
        { status: 401 },
      );
    }
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            "Add your name and details to your profile before generating a resume.",
        },
        { status: 422 },
      );
    }
    ...
    const buffer = await renderToBuffer(
      ResumeDocument({ profile, content: contentResult.data }),
    );

    const previousPath = profile.resumePdfUrl;
    ...
    if (previousPath && previousPath !== path) {
      await bucket.remove(previousPath).catch((removeError) => {
        console.error("[api/resume/generate]", removeError);
      });
    }
    ...
    if (signError || !signed) {
      ...
      return NextResponse.json(
        {
          success: false,
          error: "Resume generated, but failed to create a preview link.",
        },
        { status: 500 },
      );
    }
